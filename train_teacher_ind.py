...
def get_args():
    ...
    parser.add_argument(
        '--model_type', type=str, default='RSAGE',
        choices=['MLP', 'RSAGE', 'RGCN', 'RGAT'],
        help='Model'
    )
...
def run(args):
    ...
    if args.model_config_path is not None:
        conf = get_training_config(args.model_config_path, args.model_type, args.dataset)
    ...
    with open(conf['teacher_conf_path'], 'w') as f:
        json.dump(conf, f, indent=2)

    logger = get_logger(osp.join(output_dir, 'log_teacher.txt'))
    # logger.info = print
    logger.info(conf)
    ...
    return run_inductive(conf, obs_g, g, splits, log=logger.info)
